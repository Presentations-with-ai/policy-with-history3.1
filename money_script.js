...
const USER_NAME_BADGE_SUBTITLE_TEMP = 'Временный пользователь';
const USER_NAME_BADGE_SUBTITLE_PREMIUM = 'Премиум-пользователь';
const PREMIUM_STICKER_STATUS_KEY = `quizPremiumSticker_${QUIZ_STORAGE_NAMESPACE}_v1`;
const PREMIUM_STICKER_DEFAULT = '⭐';
...
const BANNED_USERS_CACHE_BUSTER = '2026-03-25-status-v1';
...
let currentUserHasPremiumStatus = false;
...
function getStoredQuestionRange() {
  return normalizeQuestionRange(
    localStorage.getItem(QUESTION_RANGE_START_KEY),
    localStorage.getItem(QUESTION_RANGE_END_KEY)
  );
}
...
function persistQuestionRangeSelection(range) {
  ...
  localStorage.setItem(QUESTION_RANGE_START_KEY, String(range.start));
  localStorage.setItem(QUESTION_RANGE_END_KEY, String(range.end));
}
    ...
    try {
      ...
      const rawList = Array.isArray(data)
        ...
        : Array.isArray(data?.premiumUserIds)
          ? data.premiumUserIds
          : Array.isArray(data?.bannedUserIds)
            ? data.bannedUserIds
            : [];
      ...
      console.warn('Не удалось загрузить список premium-пользователей:', error);
      return [];
    }
  })();
...
function initBanUi() {
  ...
  if (!document.getElementById('user-id-wait-overlay')) {
    ...
    waitOverlay.innerHTML = `
      <div class="agreement-panel">
        ...
        <h2 class="agreement-title">Проверка аккаунта...</h2>
  ...
  if (!document.getElementById('ban-overlay')) {
    ...
    banOverlay.innerHTML = `
      <div class="agreement-panel">
        <div class="agreement-badge">Доступ ограничен</div>
        <h2 class="agreement-title">Ваш аккаунт заблокирован</h2>
        <p class="agreement-lead">Доступ к этому боту ограничен в связи с нарушением пользовательского соглашения.</p>
      </div>
    `;
...
async function getCurrentBanStatus() {
  const [telegramMeta, premiumUserIds] = await Promise.all([
    captureTelegramWebAppUserMeta(),
    loadBannedUserIds(true)
  ]);

  const normalizedUserId = normalizeTelegramUserId(telegramMeta?.userId);
  const isPremium = normalizedUserId ? premiumUserIds.includes(normalizedUserId) : false;
  currentUserHasPremiumStatus = isPremium;
...
function isCurrentUserPremium() {
  return currentUserHasPremiumStatus === true;
}
...
function ensureUserNameBadgeStyle() {
  ...
  style.textContent = `
    ...
    #${USER_NAME_BADGE_ID} .quiz-user-name-badge-sticker.hidden,
    #${USER_NAME_BADGE_ID} .quiz-user-name-badge-action.hidden,
    #${USER_NAME_BADGE_ID} .quiz-user-name-badge-sticker-panel.hidden {
...
function normalizePremiumStickerStatus(value) {
  ...
  return matched ? matched.value : PREMIUM_STICKER_DEFAULT;
}
...
function getStoredPremiumStickerStatus() {
  try {
    ...
    return PREMIUM_STICKER_DEFAULT;
  }
}
...
function togglePremiumStickerPicker(forceState = null) {
  if (!isCurrentUserPremium()) return;
...
function renderUserNameBadge() {
  ...
  if (subtitleEl) {
    subtitleEl.textContent = isPremium ? USER_NAME_BADGE_SUBTITLE_PREMIUM : USER_NAME_BADGE_SUBTITLE_TEMP;
  }
  if (stickerEl) {
    stickerEl.textContent = currentSticker;
    stickerEl.classList.toggle('hidden', !isPremium);
  }
  if (actionButton) {
    actionButton.textContent = currentSticker;
    actionButton.classList.toggle('hidden', !isPremium);
    actionButton.disabled = !isPremium;
    actionButton.setAttribute('tabindex', isPremium ? '0' : '-1');
  }
  if (panel) {
    panel.classList.toggle('hidden', !isPremium);
    if (!isPremium) {
      panel.classList.remove('open');
      panel.setAttribute('aria-hidden', 'true');
    }
...
async function bootstrapApplicationState() {
  ...
  try {
    ...
    currentUserHasPremiumStatus = Boolean(banStatus?.premium);
    renderUserNameBadge();
...
function initializeQuestionRangeMenuInputs() {
  const storedRange = getStoredQuestionRange();
  ...
  startInput.value = Number.isInteger(storedRange.start) ? String(storedRange.start) : '';
  endInput.value = Number.isInteger(storedRange.end) ? String(storedRange.end) : '';
}
